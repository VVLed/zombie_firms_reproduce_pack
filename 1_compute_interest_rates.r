...
key_interest <- read_xlsx(paste0(path_d, "some_folder/zombie_firms/data/key_interest_rate.xlsx"), sheet = 1, skip = 0)
setDT(key_interest)

key_interest$start_date_key <- as.Date(key_interest$start_date_key)
key_interest$start_date_refin <- as.Date(key_interest$start_date_refin)

key_interest <- key_interest[year(start_date_refin) > 2010 | year(start_date_key) > 2010, ]

key_interest[, end_date_key := shift(start_date_key, type = "shift", 1) - 1]
key_interest[is.na(end_date_key), end_date_key := as.Date("2026-12-31")]  

key_interest[, end_date_refin  := shift(start_date_refin , type = "shift", 1) - 1]
key_interest[is.na(end_date_refin), end_date_refin := as.Date("2026-12-31")]  

key_interest[, key_interest_rate := as.numeric(gsub(",", ".", key_interest_rate))]
key_interest[, refin_interest_rate := as.numeric(gsub(",", ".", refin_interest_rate))]

# Create a sequence of all dates
date_seq <- data.table(date = seq(min(key_interest$start_date_refin), max(key_interest$end_date_key), by = "day"))

# For each date find appliacble rate
date_seq[, year := year(date)]
date_seq[, key_rate := key_interest[.SD, key_interest_rate, 
                      on = .(start_date_key <= date, end_date_key >= date), 
                      mult = "first"]]

date_seq[, refin_rate := key_interest[.SD, refin_interest_rate, 
                                    on = .(start_date_refin <= date, end_date_refin >= date), 
                                    mult = "first"]]

result <- date_seq[, .(key_rate = mean(key_rate), refin_rate = mean(refin_rate)), by = year]

# write_xlsx(result, paste0(path_d, "some_folder/zombie_firms/data/key_interest_rate_yearly.xlsx"))


# RUONIA ====
ruonia <- read_xlsx(paste0(path_d, "some_folder/zombie_firms/data/ruonia.xlsx"), sheet = 1, skip = 0)
setDT(ruonia)

setnames(ruonia, c("Date", "RUONIA_Year"), c("date", "ruonia_yearly"))

ruonia <- ruonia[!is.na(ruonia_yearly), .(year = year(date), date, ruonia_yearly)]

ruonia[, .N, keyby = year] # Drop 2026


ruonia <- ruonia[year != 2026, ]

ruonia[, ruonia_rate := mean(ruonia_yearly), by = year]
ruonia <- unique(ruonia[, .(year, ruonia_rate)])
write_xlsx(ruonia, paste0(path_d, "some_folder/zombie_firms/data/ruonia_rate_yearly.xlsx"))
...
# Use all-Russian deflators for regions without region-specific deflators
...
# fwrite(x = deflators, paste0(path_d, "some_folder/zombie_firms/data/ipc_deflators_2002_2025.csv"))
